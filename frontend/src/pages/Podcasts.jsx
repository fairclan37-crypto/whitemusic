...
export default function Podcasts() {
  ...
  useEffect(() => {
    fetch('/api/search?q=top%20hindi%20podcasts%20audio')
      .then(r => r.json())
      .then(d => { if (d.results) setPodcasts(d.results); })
      .catch(console.error)
    ...
    <div>
      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: 24 }}>
        <div>
          ...
          <h1 style={{ fontSize: 32, fontWeight: 800, letterSpacing: '-0.03em', display: 'flex', alignItems: 'center', gap: 12, margin: 0 }}>
            <Radio size={30} color="var(--primary)" /> Popular Podcasts
          </h1>
          <p style={{ color: 'var(--muted-foreground)', fontSize: 14, marginTop: 4 }}>
            Motivational talks, horror stories, comedy & interviews
          </p>
        </div>
        <span style={{
          ...
          <Sparkles size={14} color="var(--primary)" /> {podcasts.length} Episodes
        </span>
      </div>
      ...
      {loading ? (
        ...
        <div className="songs-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: 16 }}>
          {podcasts.map(s => (
            <SongCard key={s._id} song={s} playlist={podcasts} />
          ))}
        </div>
      )}
    </div>
